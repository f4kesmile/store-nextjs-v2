...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex" data-theme={settings.theme}>
      ...
      <aside className={cn(
          ...
          <div className="flex items-center justify-between">
            {sidebarOpen && (
              <div className="flex items-center gap-3">
                <div 
                  className="w-8 h-8 rounded-lg grid place-items-center border bg-brand-soft border-brand-primary"
                >
                  {settings.logoUrl ? (
                    <img 
                    ...
                    <Icons.dashboard className="w-4 h-4 brand-primary" />
                  )}
                </div>
                <h1 className="text-lg font-bold">{settings.storeName || "Admin Panel"}</h1>
              </div>
            )}
        ...
        <div className="p-4 border-t border-border/40 space-y-3">
          {sidebarOpen && (
            <Card className="p-3 bg-brand-soft border-brand-primary">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-brand-primary/20 rounded-full grid place-items-center">
                  {Icons.userBadge({ className: "w-5 h-5 brand-primary" })}
                </div>
      ...
      <main className={cn(
        ...
        <header className="bg-white/80 backdrop-blur-md border-b border-border/40 p-6 sticky top-0 z-30">
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center gap-3">
              <Badge variant="secondary" className="px-3 py-1 font-medium hidden sm:inline-flex">
                {session.user.role}
              </Badge>
              <Button asChild variant="outline" className="gap-2 border-brand-primary/20 hover:bg-brand-soft">
                <Link href="/">
