...
const ProductsPage: React.FC = () => {
  ...
  const columns = [
    ...
    { key: "category", label: "Kategori", render: (v: string) => <StatusBadge status={v} variant="info" /> },
  ...
  const filteredProducts = useMemo(() => products
    ...
    .filter(p => [p.name, p.sku, p.category].some(x => x.toLowerCase().includes(searchValue.toLowerCase()))),
        ...
        <div className="admin-section">
          <div className="admin-section-header">
            <div className="relative w-full sm:w-80">
              <Input placeholder="Cari nama, SKU, kategori..." value={searchValue} onChange={(e) => setSearchValue(e.target.value)} className="bg-[hsl(var(--accent))] border-[hsl(var(--border))] placeholder:text-white/40" />
            </div>
          </div>
        ...
        >
          <div className="admin-grid md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <Select value={selectedProduct?.category || "Electronics"} onValueChange={() => {}}>
