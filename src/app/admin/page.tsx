...
export default function AdminDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      const [productsRes, resellersRes, transactionsRes] = await Promise.all([
        fetch("/api/products?admin=true"), fetch("/api/resellers"), fetch("/api/transactions?limit=5"),
      ]);
  ...
  const getLowStockProducts = () => {
    ...
    return filtered.filter((p) => p.name.toLowerCase().includes(query.toLowerCase())).slice(0, 10);
  };
          ...
          <Card key={s.title} className="hover:shadow-md transition-shadow overflow-hidden">
            ...
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between">
                ...
                <Icons.pending className="w-4 h-4 text-muted-foreground" />
              </div>
                ...
                getLowStockProducts().map((product) => (
                  <div key={product.id} className="border rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2 gap-2">
                      <h4 className="font-semibold truncate max-w-[60%] sm:max-w-none">{product.name}</h4>
                  ...
                  <TableBody>
                    {recentTransactions.map((t) => (
                      <TableRow key={t.id}>
                        ...
                        <TableCell className="max-w-[180px] truncate">{t.product?.name ?? "-"}</TableCell>
                        <TableCell>
                          <Badge
                            variant={
                              t.status === "PENDING" ? "warning" : t.status === "COMPLETED" ? "success" : "secondary"
                            }
                          >
                            {t.status}
                          </Badge>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[{
              href: "/admin/products", title: "Tambah Produk", desc: "Kelola inventory", Icon: Icons.package,
            },{
              href: "/admin/resellers", title: "Kelola Reseller", desc: "Manage partners", Icon: Icons.product,
            },{
              href: "/admin/transactions", title: "Lihat Transaksi", desc: "Monitor sales", Icon: Icons.money,
            },{
              href: "/admin/settings", title: "Pengaturan", desc: "Store settings", Icon: Icons.alert,
            }].map(({ href, title, desc, Icon }) => (
              <Button asChild className="w-full p-0 h-auto" key={href}>
                <Link
                  href={href}
                  className="w-full rounded-md bg-primary text-primary-foreground hover:bg-primary/90 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                >
