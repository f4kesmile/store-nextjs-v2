...
interface Product {
  ...
  status: string;
  price: number;
  variants: Array<{ id: number; name: string; value: string; stock: number }>;
}
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const lowStockProducts = productsData.filter((product: Product) => {
        ...
        const variantStockLow = product.variants?.some(
          (v: any) => v.stock <= 3
        );
                ...
                getLowStockProducts().map((product) => (
                  <div key={product.id} className="border rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2 gap-2">
                      <h4 className="font-semibold truncate max-w-[60%] sm:max-w-none">
                        {product.name}
                      </h4>
                      ...
                      <div className="mt-2 space-y-1">
                        ...
                        {product.variants.map((variant) => (
                          ...
                          >
                            ...
                            <Badge
                              variant={getStockBadge(variant.stock).variant}
                            >
                              {variant.stock}
                            </Badge>
                          </div>
